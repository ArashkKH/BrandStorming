...
//card1 name generator
{
  ...
  function nameGen() {
    ...
    for (i = 0; i < mode; i++) {
      if (i % 2 == 1) {
        genname =
          genname + vowel[parseInt(Math.random() * (vowel.length - 1 + 1))];
      }
      if (i % 2 == 0) {
        genname =
          genname + silent[parseInt(Math.random() * (silent.length - 1 + 1))];
      }
    }
    document.getElementById("nameGen-name").innerHTML = genname;
    document.getElementsByClassName("nameGen-donBtn")[0].style.background =
      "#e47025";
    document.getElementsByClassName("nameGen-donBtn")[0].innerHTML =
      "I Like it!";
  }
  nameGen();
  ...
  document
    .getElementsByClassName("nameGen-genBtn")[0]
  ...
  // name select button-----
  document
    .getElementsByClassName("nameGen-donBtn")[0]
...
//card2 industry type select
{
  ...
  allTypes.forEach((item, index) => {
    item.addEventListener("click", () => {
      ...
      let selectedClasses = document.querySelectorAll(
        ".job-type>i.selected>span"
      );
...
//flexOrder
{
  ...
  layoutBtns.forEach((element) => {
    element.addEventListener(
      "click",
      (Event) => {
        rendercard.style.flexDirection =
    ...
    element.firstChild.addEventListener(
      ...
      (Event) => {
        rendercard.style.flexDirection =
          Event.target.parentElement.getAttribute("data-align");
...
//fetch data from sections
{
  function iconReciever(selectedIcon) {
    document
      .querySelector("#LogoCard>div>i")
      .setAttribute("class", selectedIcon);
  ...
  function setcolors() {
    ...
    document.getElementById("LogoCard").style.backgroundColor =
      pallets[1].value;
  }
}
